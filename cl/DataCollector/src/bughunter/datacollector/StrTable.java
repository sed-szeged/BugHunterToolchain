...
public class StrTable implements Serializable {
	...
	private final static Logger LOGGER = Logger.getLogger(StrTable.class.getName());
	...
	public int set(String s) {
		...
		if ((key = get(s, hashValue, bucketIndex)) == 0) {
			...
			if (count[bucketIndex] == 0xFFFF)
				LOGGER.severe("Bucket " + bucketIndex + " is full!");
	...
	public void dump() {
		for (int i = 0; i < strTable.length; i++) {
				LOGGER.info("Bucket size: " + i + " count: " + strTable[i].entrySet().size());
			for (Iterator<Map.Entry<Integer,String>> it = strTable[i].entrySet().iterator(); it.hasNext(); ) {
				Map.Entry<Integer, String> entry = it.next();
				LOGGER.info("Bucket: "+ entry.getKey() + ":" + entry.getValue());
			}
		}
	}
